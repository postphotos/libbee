...
def trend_chart(alt, mo, pl, result, summary):
    ...
    if result.is_empty():
        ...
        _metrics = ", ".join(sorted(result["metric"].unique().to_list()))
        chart = (
            ...
            .properties(width=700, height=350, title=f"Trend over time — {_metrics}")
        )
        # NB: metrics can live on different scales (e.g. checkouts/resident vs cost/visit); they share one
        # axis here, so read each line's *shape*, not cross-metric heights.
